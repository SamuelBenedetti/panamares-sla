...
export default function LangToggle({ isLight }: { isLight: boolean }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-[6px] border-[0.5px] px-[10px] backdrop-blur-[10px] hover:bg-black/5 transition-colors h-full ${
        isLight ? "border-[#0c1834]" : "border-white"
      }`}
      title={isEN ? "Cambiar a Español" : "Switch to English"}
    >
      <span className={`fi fi-pa transition-opacity duration-150 ${isEN ? "opacity-40" : "opacity-100"}`} style={{ fontSize: 20 }} />
      <span className={`font-body text-[11px] font-medium ${isLight ? "text-[#0c1834]/40" : "text-white/40"}`}>/</span>
      <span className={`fi fi-us transition-opacity duration-150 ${isEN ? "opacity-100" : "opacity-40"}`} style={{ fontSize: 20 }} />
    </button>
  );
}
