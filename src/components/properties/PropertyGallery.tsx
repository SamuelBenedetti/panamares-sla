...
export default function PropertyGallery({ images, contained = false, propertyTitle }: Props) {
  ...
  if (contained) {
    return (
      <div className="flex flex-col gap-[8px]">
        {/* Main image */}
        <div className="relative h-[300px] md:h-[420px] lg:h-[560px] w-full overflow-hidden bg-[#0c1935]">
          <Image
          ...
          {images.length > 1 && (
            <div className="absolute inset-0 flex items-center justify-between px-[16px] pointer-events-none">
              <button
                onClick={prev}
          ...
          <div className="flex gap-[8px] overflow-x-auto pb-1">
            {images.map((img, i) => (
              <button
                key={i}
                onClick={() => changeTo(i)}
                className={`relative shrink-0 w-[64px] h-[64px] overflow-hidden transition-opacity ${
                  i === active
  ...
  // Full-width mode (legacy, kept for backward compatibility)
  return (
    <div className="flex flex-col">
      ...
      {images.length > 1 && (
        <div className="bg-white shadow-[0px_4px_16px_rgba(0,0,0,0.08)] px-[30px] xl:px-[20px] 2xl:px-[120px] py-5">
