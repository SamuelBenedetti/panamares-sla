...
const BUSINESS_TYPE = "venta";
const INTENT_LABEL = "Propiedades en Venta";
const INTENT_SLUG = "propiedades-en-venta";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${properties.length} propiedades en venta en ${nbh.name}. Apartamentos, casas, penthouses y m\u00e1s en una de las mejores zonas de Panama City.`,
...
export default async function VentaNeighborhoodPage({ params }: Props) {
  const nbh = getNeighborhoodBySlug(params.neighborhood);
  if (!nbh) notFound();
  ...
  return (
    <>
      ...
      <WhatsAppButton message={`Hola, busco propiedades en venta en ${nbh.name}`} variant="floating" />
