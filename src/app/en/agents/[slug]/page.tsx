...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const role = resolveI18nString(
    agent.roleI18n,
    "en",
    agentesEn[params.slug]?.role ?? agent.role ?? ""
  );
  return {
            ...
            <div className="flex flex-col lg:flex-row gap-[16px] items-start lg:items-center mt-[8px]">
              ...
              >
                ...
                <span className="font-body font-medium text-[14px] text-white tracking-[0.35px]">
                  <span className="lg:hidden">{t.waCtaMobile}</span>
                  <span className="hidden lg:inline">{t.waCtaDesktop}</span>
                </span>
              </a>
            </div>
          </div>
              ...
              <div className="bg-white border border-[#dfe5ef] drop-shadow-[0px_1px_1px_rgba(0,0,0,0.05)] flex flex-col gap-[14px] p-[16px]">
                ...
                {agent.phone && (
                  ...
                  >
                    ...
                    <span className="font-body font-medium text-[13px] text-[#0d1835]">{t.llamarAhora}</span>
                  </a>
                )}
              </div>
            </div>
