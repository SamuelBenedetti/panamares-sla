...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!guide) return {};
  // EN-side gate — guides without a reviewed translation do not render in EN.
  if (!guide.humanReviewed) {
    return { robots: { index: false, follow: false } };
  }

  const localizedTitle = resolveI18nString(guide.titleI18n, "en", guide.title);
  ...
  return {
    ...
    alternates: {
      canonical: canonical(`/en/guides/${params.slug}`),
      languages: alternates(`/guias/${params.slug}`, `/en/guides/${params.slug}`),
    },
    openGraph: {
      title: localizedTitle,
      description: guide.excerpt,
...
export default async function GuideDetailPageEn({ params }: Props) {
  ...
  const localizedTitle = resolveI18nString(guide.titleI18n, "en", guide.title);
  const localizedBody = resolveI18nPortableText(guide.bodyI18n, "en", guide.body);
  const authorRole = guide.author
    ? resolveI18nString(guide.author.roleI18n, "en", guide.author.role)
    : "";

  const catLabel = CATEGORY_LABELS_EN[guide.category] ?? "";

  const jsonLdBreadcrumb = breadcrumbSchema([
    { name: "Home", url: "/en/" },
    { name: "Guides", url: "/en/guides/" },
    { name: localizedTitle, url: `/en/guides/${params.slug}/` },
  ]);

  const jsonLdArticle = articleSchema({
    title: localizedTitle,
    description: guide.excerpt,
    url: `/en/guides/${params.slug}/`,
        ...
        <div className="max-w-[1440px] mx-auto">
          <div className="max-w-[740px] flex flex-col gap-[48px]">
            ...
            {localizedBody.length > 0 ? (
              ...
              <p className="font-body text-[16px] text-[#5a6478]">Content coming soon.</p>
            )}
            ...
            {guide.author && (
              <div className="border border-[#dfe5ef] p-[24px] flex items-start gap-[16px]">
                ...
                <div className="flex flex-col gap-[4px]">
                  <p className="font-body font-medium text-[10px] text-[#8a95a3] uppercase tracking-[1.5px]">Written by</p>
                  <p className="font-body font-semibold text-[15px] text-[#0c1834]">{guide.author.name}</p>
                  {authorRole && (
                    <p className="font-body text-[12px] text-[#566070]">{authorRole}</p>
                  )}
                  {guide.author.credentials && (
        ...
        <div className="max-w-[1440px] mx-auto flex flex-col xl:flex-row xl:items-center xl:justify-between gap-[32px]">
          <div className="flex flex-col gap-[14px]">
            ...
            <p className="font-body font-light text-[15px] text-white/60 leading-relaxed max-w-[440px]">
              Our experts are available to guide you through every step of your real estate decision.
            </p>
          </div>
          <Link
            href="/en/contact/"
